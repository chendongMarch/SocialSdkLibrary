...
- 又有新平台？华为、OV 联运？
    - 支持扩展新平台实现，例如华为联运登录接入等；
- 每个平台支持的类型不一致，你支持音乐，我支持视频，接入迁移困难？
    - 封装各平台内部实现，对外暴露统一的接口，若不支持，使用 `web` 分享兼容；
- 本地视频无法分享？QQ 不支持纯文字分享？需要兼容方案？
    - 无法支持的数据类型，使用 `Intent` 唤醒分享，如支持本地视频分享，`qq` 的纯文字分享等等；
- 网络图片还得先下载？每次分享前还需要执行下载逻辑？
    - 支持直接使用网络图片分享，内置自动下载和图片缓存功能；
- 万一图片下载不下来怎么办？分享直接失败？
    - 图片 下载/加载 失败时降级使用资源图，避免分享被中断；
- 分享过程中需要完成统一切面操作，打点？加水印？还有啥？
    - 在分享前可统一重写数据对象，用来做分享的切面，支持图片加水印等类似需求；
- 分享增加了复制链接到粘贴板的类型，单独重新写这部分逻辑，那不是不统一了？
    - 支持短信、邮件、粘贴板等系统分享平台；
- 每次都需要打开三方 App 授权登录，能直接登录进去吗？
